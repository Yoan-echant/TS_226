function [m,chem]=viterbie_rec(numstates,compteur,tabstates,valstates,pos,len,y,s_i)
    ...
    for i=1:numstates
        if (tabstates(i,1)==pos && tabstates(i,2)==pos)
           if (mulitplie_y(y,len-compteur,numstates,valstates(i,1))< multiplie_y(y,len-compteur,numstates,valstates(i,2)))
                [metrique(i),tabchem(:,i)]=viterbie_rec(numstates,compteur+1,tabstates,valstates,i,len,y,s_i);
        ...
        elseif (tabstates(i,2)==pos)
            [metrique(i),tabchem(:,i)]=viterbie_rec(numstates,compteur+1,tabstates,valstates,tabstates(i),len,y,s_i);
            metrique(i)=metrique(i)+multiplie_y(y,len-compteur,numstates,valstates(i,2));
            tabchem(len-compteur,i)=1;
